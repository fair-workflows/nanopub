...
from nanopub.constants import FDO_DATA_REF_HANDLE_URI, FDO_PROFILE_HANDLE_URI, FDO_SERVICE_HANDLE_URI, FDO_TYPE_HANDLE_URI, FDO_STATUS_HANDLE_URI
...
def test_add_fdo_profile(fdo_profile):
    ...
    assert (fdo.fdo_uri, FDO_PROFILE_HANDLE_URI, rdflib.Literal(uri)) in fdo.assertion
    assert (FDO_PROFILE_HANDLE_URI, RDFS.label, rdflib.Literal("FdoProfile")) in fdo.pubinfo
...
def test_add_fdo_data_ref(data_ref):
    ...
    assert (fdo.fdo_uri, FDO_DATA_REF_HANDLE_URI, uri) in fdo.assertion
    assert (FDO_DATA_REF_HANDLE_URI, RDFS.label, rdflib.Literal("DataRef")) in fdo.pubinfo
...
def test_add_fdo_service(service_uri):
    ...
    assert (fdo.fdo_uri, FDO_SERVICE_HANDLE_URI, rdflib.Literal(uri)) in fdo.assertion
    assert (FDO_SERVICE_HANDLE_URI, RDFS.label, rdflib.Literal("FdoService")) in fdo.pubinfo
...
def test_add_fdo_type():
    ...
    assert (FDO_TYPE_HANDLE_URI, RDFS.label, rdflib.Literal("FdoType")) in fdo.pubinfo

def test_add_fdo_status():
    fdo = FDONanopub(FAKE_HANDLE, FAKE_LABEL)
    fdo.add_fdo_status(FAKE_STATUS)
    assert (fdo.fdo_uri, FDO_STATUS_HANDLE_URI, rdflib.Literal(FAKE_STATUS)) in fdo.assertion
    assert (FDO_STATUS_HANDLE_URI, RDFS.label, rdflib.Literal("FdoStatus")) in fdo.pubinfo
